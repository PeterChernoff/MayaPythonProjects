...
class BaseTabUI(object):
    ...
    def createUI(self, *args):
        ...
        if (self.numberOfTabs != len(self.name)):
            ...
            self.window = mc.window(self.window, title=self.title, widthHeight=self.winSize, sizeable=False,
                                    menuBar=True, mnb=True, mxb=False)

            self.mainForm = mc.formLayout(numberOfDivisions=100)
            self.tagLine = mc.text(label=" UI ")
