...
class pcCreateRigAlt05ArmsCode(object):
    ...
    def makeBndStretchTwists(self, bndJnt, crvArm, twistJnts, leftRight):
        ...
        armLen = mc.getAttr("{0}.tx".format(mc.listRelatives(bndJnt, type="joint")[0])) * imVal  # get the joint child
        # divide arm length by its base length
        mc.setAttr("{0}.input2X".format(armNrmlzDiv), armLen)
    ...
    def makeElbowCtrl(self, leftRight, ikJnts, hdlArm, *args):
        ...
        mc.move(imVal * armLength / 2, locElbow, z=True, os=True)
    ...
    def gimbalFix(self, grpArm, fkJnts, bndJnts, grpBNDConstArm, ctrlArmSettings, fkikBlendName, fkVis,
        ...
        if leftRight is CRU.valLeft:
            ...
            imVal = 1
        mc.move(imVal * sizeVal, os=True, r=True, y=True)
    ...
    def fixScaling(self, distShldrShape, distIKArmLenShape, ctrlRoot, jntShoulders, ikJnts, distArmElbowShape,
        ...
        mc.setAttr("{0}.input2X".format(gScaleArmToElbowNrmlzInvertMult), imVal)
        mc.setAttr("{0}.input2X".format(gScaleElbowToHandNrmlzInvertMult), imVal)
    ...
    def tgpMakeBC(self, mirrorSel=None, lrSel=None,
                  cbGeo=None, cbSwitchSetup=None, cbToggleSpineStretch=None, cbSpecialStretch=None,
        ...
        if cbToggleSpineStretch is None:
        ...
        if not (CRU.checkLeftRight(isLeft, bndJnt)):
            ...
            return
        bndJnts = CRU.tgpGetJnts(bndJnt, passVal, "joint", "Root Upper Arm Joint",
                                 ["JNT", "BND", "upperArm"])
        jntArmArray = bndJnts[:]

        geoJntArray = bndJnts[:]
        if cbSpecialStretch is None:
        ...
        if grpDNTTorsoCheck is None:
            ...
            passVal = "grpTorsoDNTLoad_tf"
        else:
            passVal = None
        grpDNTTorso = CRU.tgpGetTx(grpDNTTorsoCheck, passVal, "transform", "DO NOT TOUCH Torso Group",
                                   ["GRP", "DO_NOT_TOUCH", "torso"],
                                   "Group")
        if ctrlRootCheck is None:
            ctrlRootCheck = mc.textFieldButtonGrp("rootTrans_tfbg", q=True, text=True)
            passVal = "rootTrans_tfbg"
        else:
            passVal = None
        ctrlRoot = CRU.tgpGetTx(ctrlRootCheck, passVal, "nurbsCurve", "Root Control",
                                ["CTRL", "rootTransform"],
                                "control")
        if jntShoulderRootCheck is None:
            jntShoulderRootCheck = mc.textFieldButtonGrp("jointShoulderJntLoad_tfbg", q=True, text=True)
            passVal = "jointShoulderJntLoad_tfbg"
        else:
            passVal = None
        # checks if the starting joint is correct to the direction we want
        if not (CRU.checkLeftRight(isLeft, jntShoulderRootCheck)):
            ...
            return
        jntShoulderRoot = CRU.tgpGetTx(jntShoulderRootCheck, passVal, "joint",
                                       "Root Shoulder Joint",
                                       ["JNT", "BND", "shoulder"])
        ...
        if self.cbToggleSpineStretch:
            if grpSpineToggleCheck is None:
                grpSpineToggleCheck = mc.textFieldButtonGrp("grpheadShoulders_tfbg", q=True, text=True)
                passVal = "grpheadShoulders_tfbg"
            else:
                passVal = None
            grpSpineToggle = CRU.tgpGetTx(grpSpineToggleCheck, passVal, "transform",
