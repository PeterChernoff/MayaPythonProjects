...
class pcCreateRigHead(UI):
    ...
    def tgpMakeBC(self, *args):
        ...
        self.checkboxSpine = mc.checkBox("selSpineEnd_cb", q=True, v=True)
        if self.checkboxSpine:
            ctrlIKChest =  mc.textFieldButtonGrp("ctrlIKChestLoad_tf", q=True, text=True)
            grpRigSpine =  mc.textFieldButtonGrp("jointRigSpine_tfbg", q=True, text=True)
        ...
        if ((checkList[0] == "")):
            ...
            for i in range(len(self.jointArray)):
                ...
                if any(x in val for x in listCtrls):
                    if listCtrls[0] in val:
                        sizeCtrl = 11
                        orntVal = (1, 0, 0)
                        headOffsetCtrls.append(self.createCTRLs(val, prnt=False, ornt=True, size=sizeCtrl, orientVal=orntVal,  colour=18))
            ...
            if self.checkboxSpine:
                ...
                grpTorso= mc.group(n="GRP_JNT_torso", w=True, em=True)
                mc.parent(grpHead, grpRigSpine, grpTorso)
            ...
            mc.parent(eyeCtrlArray, eyesCtrl)
            ...
            self.setDriverDrivenValues(eyesCtrl, "eyesFollow", eyesCtrlPrntConst, eyesFollow[0], 0, 1)
            self.setDriverDrivenValues(eyesCtrl, "eyesFollow", eyesCtrlPrntConst, eyesFollow[1], 0, 0)

            self.setDriverDrivenValues(eyesCtrl, "eyesFollow", eyesCtrlPrntConst, eyesFollow[0], 1, 0)
            self.setDriverDrivenValues(eyesCtrl, "eyesFollow", eyesCtrlPrntConst, eyesFollow[1], 1, 1)


            for i in range(len(eyesFollowLocArray)):
                mc.setAttr("{0}.v".format(eyesFollowLocArray[i]), False)

            grpWorldFollow = mc.group(n="GRP_LOC_worldFollow",w=True, em=True)
            ...
            if self.checkboxSpine:
                ...
                for i in range(len(headSpaceFollow)):
                    ...
                    self.setDriverDrivenValues(headCtrl, "headFollow", headFollowOrntConstr, headSpaceFollow[i], i, 1)
                    for i2 in range(len(headSpaceFollow)):
                        if i2!=i:
                            self.setDriverDrivenValues(headCtrl, "headFollow", headFollowOrntConstr, headSpaceFollow[i2], i2, 0)
            ...
            for i in range(len(headOffsetCtrls)):
                self.lockHideCtrls(headOffsetCtrls[i][1],translate=True, scale=True)

            for i in range(len(eyeCtrlArray)):
                self.lockHideCtrls(eyeCtrlArray[i], scale=True)

            self.lockHideCtrls(eyesCtrl, scale=True)
